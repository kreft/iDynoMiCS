...
public class LocatedGroup
{
	...
	/**
	 * \brief Set the coordinates of this group and check these are inside the
	 * grid. 
	 *  
	...
	/**
	 * Reset the move vector to all zeros.
	 */
	 ...
	 * 
	 * TODO RC - give the option to specify the reason for death.
	 */
	public void killAll()
	{
	...
	{
		Integer delta = Integer.signum(i);
	...
	{
		Integer delta = Integer.signum(j);
	...
	{
		Integer delta = Integer.signum(k);
								...
								ContinuousVector him, ContinuousVector move)
	{
		...
		if (agentGrid.is3D)
		{
			...
			if (Math.abs(move.z) > 0.5*gridLength)
