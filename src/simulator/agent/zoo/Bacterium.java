...
{	
	...
	/**
	 * Boolean noting whether any inert particles have been declared as part of
	 * this Bacterium.
	 */
	...
	public void initFromProtocolFile(Simulator aSim, XMLParser aSpeciesRoot) 
	{
		...
		// Set cell radius, generation, and genealogy.
		init();
	}
	...
	public void mutatePop() 
	{
		...
		if (getSpeciesParam().distMethod)
			...
			for (int i = 0; i < particleMass.length; i++)
				particleMass[i] = ExtraMath.deviateFromCV(1.5*particleMass[i],
											 getSpeciesParam().initialMassCV);
	}
	...
	/**
	 * \brief Called at each time step of the simulation to compute mass growth
	 * and update radius, mass, and volume.
	 * 
	 ...
	 * the size at which it must divide, and monitors agent death.
	 */
	...
	/**
	 * \brief Excretes EPS particle with 75% of initial mass if the EPS capsule
	 * is too thick.
	 */
	...
	public void excreteEPS(Double ratio)
	{
		...
		// Check the mass to excrete exists.
	...
	/**
	 * \brief Determines if this agent has reached either the radius size limit
	 * at which it will die, or a state of zero mass.
	 * 
	 * @return Boolean value noting whether the cell will die (true) or not
	 * (false).
	 */
	 ...
	 * the mass at-division.
	 * 
	 * This sets the mass of the primary particle of the progenitor to half the
	 * mass at-division (i.e. the average mass after division, regardless of
	 * babyMassFrac). The mass-at-division is determined by the particle
	 * density and the volume-at-division; volume is determined by the
	 * divRadius, or the divRadius and the z-resolution if in a 2D simulation.
	 */
