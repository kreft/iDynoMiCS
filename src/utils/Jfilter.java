...
/**
 * \brief Used in simulation initialisation to ensure only XML files are read
 * as protocol files and previous results files are excluded.
 * 
 * This can occur when a simulation is being restarted.
 */
public class Jfilter implements FilenameFilter 
{
	...
	/**
	 * \brief Initialised the file extension filter and sets the extension to
	...
	/**
	 * \brief Used to determine if a file should be excluded from the filter
	 * or whether the file name is acceptable.
	 * 
	 * @param directory	The directory of files being filtered.
	 * @param filename	The filename being checked against the filter.
	 * @return Boolean value noting whether or not the file ends with the
	 * extension.
	 */
	@Override
