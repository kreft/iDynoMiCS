...
*
* Launch iDynomics using this class. Syntax:
* - To launch a single simulation
* 		idyno.Idynomics \\protocol\\protocolFileURL
* - To launch a batch of simulations
* 		idyno.Idynomics \\protocol\\FilesDirectory
* 
* Please make sure you are aware of the licensing agreement when you are
* running the iDynomics tool. This software is governed by the CeCILL license
* under French law and  abiding by the rules of distribution of free software.
* You can use, modify and/or redistribute the software under the terms of the
* CeCILL license as circulated by CEA, CNRS and INRIA at http://www.cecill.info 
* 
* @since June 2006
* @version 1.2
...
public class Idynomics
{
	...
	/**
	 * Text panel within the update frame that takes text from a URL on the
	 * iDynomics server (history.html) to display what changes have been made
	 * in this update.
	 */
	private static JEditorPane infoPane;
	
	/**
	 * Scroll pane to contain the description of the changes, should these be
	 * numerous.
	 */
	...
	/**
	 * The program arguments sent in with this run (which have yet to be
	 * processed) - passed round such that these can be processed
	 * after the update check.
	 */
	public static String[] programArgs;
	
	/**
	 * Store new version number (if there is one) globally as required by
	 * action handlers.
	 */
	public static Double newVersionNumber;
	
	/**
	 * An array of the protocol XML file names to be processed, each
	 * describing a particular simulation scenario.
	 */
	...
	/**
	 * A double used to calculate how long the simulation has taken to run
	 * under a set of specified conditions.
	 */
	public static long begin;
	
	/**
	 * A simulation object that runs the simulator under the conditions
	 * specified in the protocol file.
	 */
	public static Simulator aSimulator;
	
	/**
	 * When this is true, only the absolute essentials are displayed and
	 * written to the log file.
	 */
	public static Boolean quietMode = false;

	/**
	 * The simulator needs this to read/write the random number file.
	 */
	public static String currentPath;
	
	/**
	 * Log to hold any issues with simulation initialisation. Created as
	 * initialisation happens prior to creation of any log file.
	 */
	 ...
	 * 
	 * This method starts iDynoMiCS and launches the required methods to
	 * process the protocol files provided by the user. Firstly, these command
	 * line or Eclipse dialog box arguments are processed, such that the tool
	 * is aware of the protocol files it needs to process. With this complete,
	 * the tool performs a check that tests whether it is the latest stable
	 * release of iDynoMiCS. If this is the case, the simulation starts as
	 * normal. If not, the user has the opportunity to update iDynoMiCS and
	 * relaunch the tool. Note that this check can be turned off in the
	 * protocol file, and thus a further check is included in this file to
	...
	public static void main(String[] args) 
	{
		...
		try
		{
			...
			/* Now set the current path to that of the first protocol file to
			 * be processed (activePath is set by processArguments).
			 */
			...
			/*
			 * The first thing we're going to do is check for an iDynoMiCS
			 * stable release update - if the user has specified this. The
			 * user can decide whether this is done or not in the protocol
			 * files they are submitting to iDynoMiCS.
			 * 
			 *  KA - for sake of saving checking time and avoiding problems
			 *  with bulk runs, the decision has been made to only check the
			 *  FIRST protocol file. Although this could be deemed a bit risky,
			 *  as the user may select a check in the other protocol files,
			 *  this is important, as checking every file will (a) take time,
			 *  and (b) could cause the run of simulations to be halted if the
			 *  first few all say don't check for updates, yet one protocol
			 *  file then says to. So we should be instructing users that if
			 *  they do wish to check for updates, this needs to be noted in
			 *  the first protocol file that they specify. The below method
			 *  checks the protocol file, and if update check is enabled,
			 *  performs the check as to whether this is the latest version.
			 *  If not, this returns true. If it is the latest version or the
			 *  update check is set to false, false is returned
			 */
			Boolean updateCheck = 
					checkForReleaseUpdate(_activePath[0],_protocolFile[0]);
			
			/*
			 * Call the method to show the update box and offer the user the
			 * chance to update.
			 */
			if(updateCheck)
				show_Update_Box(args, Check_Release_Status.getWhatsNew());	
			/* 
			 * Start processing the protocol files. Note this has been
			 * included as a call rather than the code be placed here, as the
			 * start must be accessible from the update screen too - as the
			 * user may choose not to update. 
			 */
			else 
				startIDynomics();
	 ...
	 * 
	 * This method creates a dialog box on the screen that shows that an
	 * update is available for the iDynomics package. This dialog box will
	 * contain information from a file history.html, on the iDynomics web
	 * server, that details the changes included within this new version. The
	 * user is given two options - to either install the update (after which
	 * they will need to restart iDynomics), or proceed with the simulation
	 * without installing the update.
	 * 
	 * @param args				The program arguments sent in to iDynomics.
	 * @param updateInfoMessage	The list of changes in this update, from
	 * history.html
	 */
	public static void show_Update_Box(String[] args, String updateInfoMessage)
	{
		/* Store the program arguments so these can be processed if an update
		 * is not performed.
		 */
		programArgs = args;
		...
		/*
		 *  Create the Update button, and set this to start the jar that
		 *  performs the update. Note that the jar must be in the updater
		 *  folder.
		 */
		...
		ok.addActionListener( new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e) 
			{
				...
				/*
				 *  Exit once this is started, as the updater may not be able
				 *  to update some of the running classes if this is still
				 *  running.
				 */
				System.exit(0);
			}
		...
		/*
		 * Add a cancel button in case the user does not want to run the 
		 * update. If this is the case iDynomics will start.
		 */
	 ...
	 * 
	 * This method iterates through each of the protocol files provided and
	 * launches a simulation under each of these conditions. Once this is
	 * complete, the random number state file for that simulation is also
	 * written to file. Note that from version 1.2 this method is new - but
	 * all this code previously existed in the main method. It has been moved
	 * here to work with the updater - the user may get the update screen but
	 * choose not to update. If this is the case, they need to be located
	 * somewhere where they can launch the simulation. This was only possible
	...
	/**
	 * \brief Method to select protocol files from a file selection dialog if
	 * run in Eclipse.
	 * 
	 * When run from the command line (using the RunIDyno.py file), the
	 * simulation expects XML protocol file names to be provided. If however
	 * the simulation is run from Eclipse (which is possible, especially for
	 * those developing it), the user would have to either specify the file
	 * name as a Run Argument, or provide this during RunTime. This method
	 * allows the latter, presenting the simulation user with a selection box
	 * from which to choose one or more protocol files on which simulations
	 * should be run. These are returned within a linked list for later 
	 * processing.
	 * 
	 * @return Linked list of XML files selected from the dialog box.
	 */
	...
	/**
	 * \brief Processes the command line or Eclipse protocol file selection
	 * arguments, forming a list of simulations to be run.
	 * 
	 * iDynoMiCS can be run in a couple of different ways. Yet both require
	 * the user to specify one or a set of XML protocol files that describe
	 * the simulation that will be run. This method is used in both Eclipse
	 * and command line version of iDynoMiCS, and processes this input. For a
	 * command line run (from RunIDyno.py for example), the user will specify
	 * the protocol files on the command line. For an Eclipse run, these will
	 * be specified via a file selection dialog box. Either way, these are
	 * then processed into two string arrays, one containing the paths to the
	 * protocol files to be run and another containing the names of the
	 * protocol files. Once this is complete. iDynoMiCS can be started such
	 * that each is processed in turn.
	 * 
	 * @param args	The protocol file names specified on the command line (not
	 * used in an Eclipse run).
	 */
	static public void processArguments(String[] args) 
	{
		LinkedList<File> allFiles;
		/*
		 * We're going to build a list of the protocol files upon which
		 * simulations will be run. As there are two ways of running the
		 * simulation (from the RunIDyno script where a protocol file must be
		 * provided, and Eclipse where a protocol file can be selected from a
		 * window) we need to check whether a protocol file (or files) have
		 * been provided as arguments.
		 */
		switch (args.length) 
		{
			case 0:
				/* The simulation is being run in Eclipse. A protocol file (or
				 * files) need to be specified - and this is done using a file
				 * selection window.
				 */
				allFiles = chooseFile();
				break;
			default:
				/* The assumption here is that protocol files have been
				 * provided as command line arguments. RunIDyno.py checks that
				 * this is the case before the simulation is run. Initialise
				 * the linked list (if wondering why not done in above case,
				 * this was processed in chooseFile).
				 */
				allFiles = new LinkedList<File>();
				
				/* Now go through the arguments, and process each using the
				 * listDirectory utility method (could be a directory or file).
				 */
				for (String arg : args)
					listDirectory(new File(arg), "xml", allFiles);
		}
		
		/* Now to remove any agent_State and env_Sum files from the list.
		 * KA 26/03/13 - When I looked at this I saw no requirement to do this
		 * (as the files were never there, as the XML filter made sure of this).
		 * I have however left this here for the moment - this should be
		...
		/* Now to build the list of path and protocol files that will be 
		 * processed in this run. Initialise the arrays that will store
		 * (a) the paths to the protocol files
		 * (b) the names of the protocol files 
		 */
		_activePath = new String[nFile];
		_protocolFile = new String[nFile];
	 ...
	 * 
	 * Launches a simulation for each of the provided protocol files, and logs
	 * how long the simulation has taken to run under those conditions.
	 * 
	...
	/**
	 * \brief Processes the files and directories selected from the dialog box
	 * when iDynoMiCS is run in Eclipse.
	 * 
	 * When iDynoMiCS is run, the user has to specify protocol files. This is
	 * done either via a dialog box in Eclipse, or from the command line if
	 * run from RunIDyno.py. This method takes the files and directories that
	 * have been specified either way and processes these, adding the relevant
	 * files to a linked list. There are a few considerations here:
	 * (a) If only one file is selected, the method checks this is the correct
	 *     file type (specified by input argument filter) and if so adds this
	 *     to the list.
	 * (b) If the selection is a directory, the whole directory is added to
	 *     the list, again after a pre-check that the files are of the correct
	 *     file format.
	 * 
	 * @param aDirectory	The file(s) or directory selected from the dialog
	 * box or specified on the command line for processing.
	 * @param filter	The file format of the protocol files (basically the
	 * filter to apply to these files).
	 * @param fileList	The linked list that the protocol files should be added
	 * to.
	 */
	public static void listDirectory(java.io.File aDirectory, String filter, 
	 ...
	 * Andreas D̦tsch, Barth F Smets, Cristian Picioreanu, Rob Clegg, Sonia Martins, Katrin Bohl, Kieran Alden, Jan-Ulrich Kreft 
	 * and others.
	 *   
	 *   <br>
