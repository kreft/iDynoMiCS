...
public class SimTimer
{
	...
	/**
	 * If the timestepping is adaptive, we keep a record of the previous
	 * timesteps used.
	 */
	...
	/**
	 * \brief Parses simulation time step information from the XML protocol
	 * file.
	 * 
	...
	/**
	 * \brief Computes the current simulation time step and increments the
	 * simulation clock.
	 * 
	 ...
	 * 
	 * @param aWorld The simulation world being modelled under an adaptive
	 * timestep.
	 */
	public static void updateTimeStep(World aWorld)
	{
		...
		_dT = Math.floor(10*_dT/_dTMin) * _dTMin / 10.0;
	...
	/**
	 * \brief Set the state of the simulation timer from information in an XML
	 * file.
	 * 
	 * @param infoFile	Name of the XML file containing the sim timer
	 * information.
	 */
	public void setTimerState(String infoFile)
	{
		...
		XMLParser localRoot =
						new XMLParser(fileRoot.getChildElement("simulation"));
	...
	/**
	 * \brief Determines if a set time (such as an agent birthday) is within
	 * the next timestep.
	 * 
	 * Used to determine when agents enter the simulation.
	 * 
	 * @param aDate	The timestep at which an event is set to happen.
	 * @return	Boolean stating whether this event should occur in the next
	 * timestep.
	 */
	 ...
	 * 
	 * Useful for determining whether an event can occur in a set window (e.g.
	 * cell entry).
	 * 
	 ...
	 * timestep.
	 */
	public static Boolean isCurrentTimeStepInRange(Double start, Double end)
	{
