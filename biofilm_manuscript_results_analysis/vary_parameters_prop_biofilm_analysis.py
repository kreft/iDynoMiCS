...
def get_colorbar(ax, color, name, side):
    pad=0.01
    plt.sca(ax)
...
get_colorbar(colbar, 'autumn', 'Adaptive repair', 'left') #AR
colbar = plt.subplot(3,16,30, frameon=False)
get_colorbar(colbar, 'cool', 'Damage segregation', 'left') #ASNR
colbar = plt.subplot(3,16,31, frameon=False)
get_colorbar(colbar, 'gray', 'Solute', 'left') #ASNR
...
for o in range(len(original)):
    plot_biofilm(ax_orig[o], main_folder+original[o])
    plt.sca(ax_orig[o])
...
for a in range(len(others)):
    ...
    for b in range(len(others[a])):
        ...
        plot_biofilm(this_ax, main_folder+others[a][b])
...
for a in range(len(sets)):
    ...
    if a == 0:
        this_ax[0].text(-0.2, 0.5, 'Log(AR/DS)\nbiomass', rotation=90, ha='left', va='center', transform=this_ax[0].transAxes)
...
os.chdir('/Users/robynwright/Documents/GitHub/iDynoMiCS_1.5/biofilm_manuscript_results_analysis/vary_parameters')
plt.savefig('Log biomass ratios sbs.png', bbox_inches='tight', dpi=600)
...
for a in range(len(sets)):
    ...
    for b in range(len(fol)):
        if sets[a] in fol[b] and '.gif' not in fol[b]:
            ...
            plot_biofilm(ax, fol_name+fol[b])
