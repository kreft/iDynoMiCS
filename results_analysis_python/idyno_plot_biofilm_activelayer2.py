        ...
        for cell in species.members:
            ...
            for (species_name, colormap_name) in species_color_dict.iteritems():
                ...
                if species.name == 'OldieA':
                    if growthrate == 1: cell.color = '#f200dc'
                    else: cell.color = 'red'
                elif species.name == 'OldieB':
                    if growthrate == 1: cell.color = '#f0f200'
                    else: cell.color = '#f2ab00'
...
def plot(iter_info, min_max_concns, plot):    
    ...
    color_cells_by_growth_and_species(iter_info.agent_output, species_color_dict)
    toolbox_idynomics.plot_cells_2d(axis, iter_info.agent_output)
        
    if sp == 1:
        divider = make_axes_locatable(axis)
        cax = divider.append_axes("right", size="30%", pad=0.2)
        ages, locationX, prep, agestd, prepstd = get_agegrid()
        ...
        if plot == 4:
            cax.set_xlabel(r'Mean Age and $P_{rep}$/$P_{tot}$')
    ...
    axis.plot([left + 10], [y1], 'o', color='#f200dc', markeredgecolor='none', markersize=14)
    axis.text(left + 20, y1, 'Asymmetric No Repair Active Layer', va='center', ha='left', fontsize=14)
    axis.plot([left + 10], [y2], 'o', color='red', markeredgecolor='none', markersize=14)
    axis.text(left + 20, y2, 'Asymmetric No Repair', va='center', ha='left', fontsize=14)
    
    axis.plot([left + 10], [y3], 'o', color='#f0f200', markeredgecolor='none', markersize=14)
    axis.text(left + 20, y3, 'Symmetric Adaptive Repair Active Layer', va='center', ha='left', fontsize=14)
    axis.plot([left + 10], [y4], 'o', color='#f2ab00', markeredgecolor='none', markersize=14)
    axis.text(left + 20, y4, 'Symmetric Adaptive Repair', va='center', ha='left', fontsize=14)     
    ...
    if plot == numplots-1:
        ...
        save_path = os.path.join(sim.figures_dir, save_name+'_growthactivelayer'+save_num+options.file_ext)
        if not options.substratum:
                save_path = os.path.join(sim.figures_dir, save_name+'_'+save_num+'_growthactivelayer'+options.file_ext)
