...
paths = ['T010AS12NRConstEnvCleggComp_life_history_results.xml', 'T010AS12007ConstEnvCleggComp_life_history_results.xml',
         'T010S12NRConstEnvCleggComp_life_history_results.xml', 'T010S12007ConstEnvCleggComp_life_history_results.xml', 'T010AS12OBConstEnv1_0D_Ind_life_history_results.xml']
...
paths = ['T010AS12OBConstEnv1_0D_Ind_life_history_results.xml']
colorsC = ['#fbbf07']
attributes = {'name':'specific growth rate', 'name2':'age', 'header':'time,value,value2'}
...
def calc_beta(age):
    #beta for toxic
    return (( age / (1 - age))  * ( abs(math.sqrt((Yr / muS) * (1 / (1 - age))) - 1)))
...
for i in range(0, len(val)-1):
    if [val[i]] < 0:
            break
    if (val[i+1] - val[i] > 0.05):
        division.append(t[i])
    x.append(t[i])
    y.append(calc_beta(val[i]))    
axisC.plot(x, y, color=colorsC[0])
for j in range(0,len(division)):
    axisC.plot(division[j], 0, 'r*')
        
fs = 8 

#axisC.set_xlim([0, xlim])
axisC.set_xticks([0, 4, 8, 12, 16, 20])#, 24])
#axisC.set_ylim([0, ylim])
