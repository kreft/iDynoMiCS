...
for iter_info in sim_dir.get_iterate_information():
    act_grow = float(iter_info.agent_output.species_outputs[0].members[0].vars['activeBiomassGrowth'])
    act_repair = float(iter_info.agent_output.species_outputs[0].members[0].vars['activeBiomassRepair'])
    inact_grow = float(iter_info.agent_output.species_outputs[0].members[0].vars['inactiveBiomassGrowth'])
    inact_repair = float(iter_info.agent_output.species_outputs[0].members[0].vars['inactiveBiomassRepair'])
    ptot = act_grow + act_repair + inact_grow + inact_repair
    prep = act_repair + inact_repair
    prep_ptot = prep / ptot
    prep_ptot_list.append(prep_ptot)

    age = float(iter_info.agent_output.species_outputs[0].members[0].vars['age'])
    #beta for toxic
