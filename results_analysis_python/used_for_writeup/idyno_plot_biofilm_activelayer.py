...
def plot(sim, plot):
    ...
    for i in range(TP):
        ...
        axis.plot([left + 10], [y1], 'o', color='red', markeredgecolor='none', markersize=56)
        axis.text(left + 20, y1, 'Species 1 Active Layer', va='center', ha='left', fontsize=56)
        axis.plot([left + 10], [y2], 'o', color='#ff69b4', markeredgecolor='none', markersize=56)
        axis.text(left + 20, y2, 'Species 1', va='center', ha='left', fontsize=56)
        
        axis.plot([left + 10], [y3], 'o', color='#f68a04', markeredgecolor='none', markersize=56)
        axis.text(left + 20, y3, 'Species 2 Active Layer', va='center', ha='left', fontsize=56)
        axis.plot([left + 10], [y4], 'o', color='#f6ec04', markeredgecolor='none', markersize=56)
        axis.text(left + 20, y4, 'Species 2', va='center', ha='left', fontsize=56)        
        ...
        if plot == numplots-1 and i == TP-1:
            figure.subplots_adjust(left=lb, bottom=lb, right=0.9, top=0.9, wspace=0.9, hspace=0.9)
            figure.process_subplots()
            figure.inset_axes()
            save_num = str(i_info.number)
            save_num = (num_digits - len(save_num))*'0' + save_num
            save_path = os.path.join(sim.figures_dir, save_name+'_'+save_num+'_activelayer'+options.file_ext)
            if not options.substratum:
                save_path = os.path.join(sim.figures_dir, save_name+'_'+save_num+'_activelayer_nosubstrate'+options.file_ext)
