...
if options.figure_type == None:
    ...
    else: height = toolbox_plotting.mm2inch(nI * res * (numplots+3))
    if options.width > 0: width = (options.width*TP)
    else: width = toolbox_plotting.mm2inch(nJ * res*2)
...
def get_agegrid(iter_info):
    ...
    for r in iter_info.agent_output.get_all_cells():
        ...
        pag = float(r.vars['activeBiomassGrowth'])
        par = float(r.vars['activeBiomassRepair'])
        pdg = float(r.vars['inactiveBiomassGrowth'])
        pdr = float(r.vars['inactiveBiomassRepair'])
        prep = par+pdr
        ptot = pag+par+pdg+pdr
        prepptot = prep/ptot
        ageAll[x] = ageAll[x] + [age]
        pRepAll[x] = pRepAll[x] + [prepptot]
    ...
    for a in range(maximum):
        ...
        if len(pRepAll[a]) > 0:
            pRepAll2[a] = numpy.mean(pRepAll[a])
            pRepSD[a] = numpy.std(pRepAll[a])
    return ageAll2, locationX2, pRepAll2, ageSD, pRepSD

 
def get_axes(numplots, plot, tp):
    col = TP*10  
    row = (numplots*100) + 100
    tp += 1
    plot += 1
    num = plot*TP-(TP-tp)
...
def plot(sim, plot):
    ...
    for i in range(TP):
        ...
        color_cells_by_age_and_species(i_info.agent_output, species_color_dict)
        ...
        ages, locationX, prep, agestd, prepstd = get_agegrid(i_info)
        for i in range(len(ages)):
            SD = ([(ages[i]-agestd[i]),(ages[i]+agestd[i])])
            locX = ([locationX[i], locationX[i]])
            cax.plot(SD, locX, color='#515c5c')
        cax.plot(ages, locationX, 'r',label='Age')
        for j in range(len(prep)):
            SD = ([(prep[j]-prepstd[j]),(prep[j]+prepstd[j])])
            locX = ([locationX[j], locationX[j]])
            cax.plot(SD, locX, color='#515c5c')
        cax.plot(prep, locationX, 'g',label=r'$P_{rep}$/$P_{tot}$')
        cax.set_xlim(0,1)
        cax.set_xticks([0, 0.5, 1.0])
        cax.set_yticks([])
        cax.set_xlabel(r'Mean Age and $P_{rep}$/$P_{tot}$', fontsize=18)
        cax.set_ylim(0, nI * res)
        cax.legend(bbox_to_anchor=(1.05, 1), loc='upper right', borderaxespad=0., fontsize=18)
        ...
        if options.frameon:
            ...
            axis.set_ylabel(r'y ($\mu$m)', fontsize=18)
        if options.timeon:
            axis.text(0.1*res, ((nI+0.1)*res-8), 'Time: %d h'%(int(i_info.time)),
                      va='bottom', ha='left', color='#bd0303', fontsize=20)
        xmax = nJ * res
        axis.set_xlim(0, xmax)
        axis.set_ylim(0, nI*res)
        axis.set_xlabel(r'x ($\mu$m)', fontsize=18)
        #axis.set_ylim(0, 50)
        if numplots > 1 and plot == 0:
            axis.set_title('No repair', fontsize=24, loc='left')
            #if not options.titlescript is None:
                #axis.set_title(options.titlescript, fontsize = 16, loc='right', x= -1)
        if plot == 1:
            axis.set_title(r'$\beta $= 0.06', fontsize=24, loc='left')
            axis.set_title('', fontsize = 24, loc='center')
        if plot == 2:
            axis.set_title(r'$\beta $= 0.07', fontsize=24, loc='left')
            axis.set_title('', fontsize = 24, loc='center')
        if plot == 3:
            axis.set_title(r'$\beta $= 0.08', fontsize=24, loc='left')
            axis.set_title('', fontsize = 24, loc='center')
        if plot == 4:
            axis.set_title('Adaptive repair', fontsize=24, loc='left')
            axis.set_title('', fontsize = 24, loc='center')
        if plot == 4:
            axisA = figure.add_subplot('', 111, frameon=False)
            axisA.set_title(options.titlescript, fontsize=24, loc='right', x=-1)
        wspace = 0.1
        hspace = 0.3
        if not nI == 65:
            hspace=0.3
        if plot == numplots-1:
            ...
            save_path = os.path.join(sim.figures_dir, save_name+'_'+save_num+'_ageallfixedbeta'+options.file_ext)
