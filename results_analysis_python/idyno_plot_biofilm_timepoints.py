...
if options.figure_type == None:
    ...
    else: height = toolbox_plotting.mm2inch(nI * res * numplots*7)
    if options.width > 0: width = (options.width*TP*2)
    else: width = toolbox_plotting.mm2inch(nJ * res * TP * 2)
...
growthvec=[]     
species_color_dict = {'OldieA' : 'Spectral', 'OldieB' : 'Greens', 'OldieC' : 'hot'}
...
def colorbar(axis, position="top", s=1, label='Specific growth rate'):
    cmap = matplotlib.cm.get_cmap('Spectral', 256)
    a = numpy.array([[0,0.08]])
    norm = matplotlib.colors.Normalize(vmin=0, vmax=0.25)
    m = matplotlib.cm.ScalarMappable(norm=norm, cmap=cmap)
    m.set_array(a)
    fontsize=30
    cbar = toolbox_plotting.make_colorbar(axis, m, side=position, fontsize=30, label=label)
    ticks=([0, 0.05, 0.1, 0.15, 0.2, 0.25])
    cbar.set_ticks(ticks, update_ticks=True)
    cbar.set_label('Specific Growth Rate', size=fontsize)
    return cbar
...
def plot(sim, plot):
    ...
    for i in range(TP):
        ...
        if i == 0:
            newaxis = figure.add_subplot('', 335, frameon=False)
            cbar = colorbar(newaxis, position="top")
        ...
        if options.frameon:
            ...
            axis.set_ylabel(r'y ($\mu$m)', fontsize=30)
        if options.timeon:
            axis.text(0.1*res, ((nI+0.1)*res-8), 'Time: %d h'%(int(i_info.time)),
                      va='bottom', ha='left', color='#bd0303', fontsize=40)
        roughness = calc_roughness.calc_roughness(options.results_dirr, best)
        axis.text(250, ((nI+0.1)*res-8), r'$\sigma_{f}$ = %f'%float(roughness), va='bottom', ha='right', color='#bd0303', fontsize=40)
        xmax = nJ * res
        axis.set_xlim(0, xmax)
        axis.set_ylim(0, nI * res)
        axis.set_xlabel(r'x ($\mu$m)', fontsize=30)
        #axis.set_xticks([0, 50, 100, 150, 200, 250], fontsize=25)
        #axis.set_yticks([0, 50, 100, 150, 200, 250], fontsize=25)
        #axis.set_ylim(0, 50)
        if i == 0:
            axis.set_title('A', fontsize=40, loc='left')
