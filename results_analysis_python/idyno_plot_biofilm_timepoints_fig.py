...
if options.figure_type == None:
    ...
    else: height = toolbox_plotting.mm2inch(nI * res * (numplots+1))
...
def get_axes(numplots, plot, tp):
    ...
    num = plot*TP-(TP-tp)
    if numplots*TP > 9:
        axistouse = figure.add_subplot(row, col, num)
    else:
...
def plot(sim, plot):
    ...
    for i in range(TP):
        ...
        axis2 = get_axes(numplots, plot, i)
        if numplots*TP > 9: axis = plt.plot
        else: axis = axis2
        color_cells_by_age_and_species(i_info.agent_output, species_color_dict)
        min_max_concns = i_info.get_min_max_concns()
        toolbox_idynomics.plot_cells_2d(axis2, i_info.agent_output)
        ...
        if numplots > 1 and plot == 0:
            ...
            if numplots*TP > 9: fs = 20
            else: fs = 16
            if not options.titlescript is None and not i > 0:
                axis.set_title(options.titlescript, fontsize = fs, loc='center', x= -1)
        if plot == 1:
        ...
        wspace = 0.1
        hspace = 0.4
        if plot == numplots-1 and i == TP-1 and numplots*TP > 9:
            plt.show()
            plt.tight_layout()
        else:
            if not nI == 65:
