...
if options.colorscheme==1:
    species_color_dict = {'OldieA' : 'hsv', 'OldieB' : 'cool', 'OldieC' : 'cool'}
else:
    species_color_dict = {'OldieA' : 'hsv', 'OldieB' : 'cool', 'OldieC' : 'hsv'}
...
def plot(iter_info, min_max_concns, plot):    
    ...
    if sp == 1:
        ...
        if plot == 0:
            cax.legend(bbox_to_anchor=(0.35, 0.975), loc=2, borderaxespad=0.)   
    ...
    if plot == numplots-1:
        ...
        save_path = os.path.join(sim.figures_dir, save_name+'_initiation_'+save_num+options.file_ext)
