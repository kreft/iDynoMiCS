...
file_path = os.path.join(dir_path, 'agent_State_spacing_halfhalf_64_200.xml')
gridres = 4
density = 201
genealogy = 0.0
generation = 0.0
...
script += '</species> \n'
script += '<species name="OldieB" header="family,genealogy,generation,birthday,activeBiomassGrowth,activeBiomassRepair,inactiveBiomassGrowth,inactiveBiomassRepair,growthRate,volumeRate,locationX,locationY,locationZ,radius,totalRadius,birthX,birthY,birthZ,age,hasDied,deathX,deathY,deathZ,deathday">\n'
for i in range(1):
    locationY = 200
    activeBiomassG = activeBiomassGrowth * math.pow(2, random.random())
    radius = math.sqrt(activeBiomassG/(density*gridres*math.pi))
