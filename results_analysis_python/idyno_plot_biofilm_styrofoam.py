...
import toolbox_plotting_RW as toolbox_plotting
...
if options.figure_type == None:
    ...
    else: height = toolbox_plotting.mm2inch(nI * res * res)
...
def plot(iter_info, min_max_concns, plot, last_iter):    
    ...
    if not options.titlescript is None:
        axis.set_title(options.titlescript, loc='left')
    if options.frameon:
    ...
    if options.timeon:
        axis.text(0.1*res, ((nI+0.1)*res-8), 'Time: %d h'%(int(iter_info.time)),
                  va='bottom', ha='left', color='#bd0303', fontsize=18)
    '''
    if plot == 0:
        roughness = calc_roughness.calc_roughness(options.results_dirr, last_iter)
    if plot == 1:
        roughness = calc_roughness.calc_roughness(options.results_diRr, last_iter)
    if plot == 2:
        roughness = calc_roughness.calc_roughness(options.results_diRR, last_iter)
    axis.text(250, ((nI+0.1)*res-8), r'$\sigma_{f}$ = %f'%float(roughness), va='bottom', ha='right', color='#bd0303', fontsize=18)
    '''
    xmax = nJ * res
    ...
    if numplots > 1 and plot == 0:
        ...
        if not options.titlescript is None:
            axis.set_title(options.titlescript, fontsize = 20, loc='center', x = -0.1)
    if plot == 1:
    ...
    if plot == numplots-1:
        ...
        save_path = os.path.join(sim.figures_dir, save_name+'_styrofoam_'+save_num+options.file_ext)
...
for j in range(numplots):
    ...
    if options.all_iter:
        ...
        if options.zero_color:
            min_max_concns[options.solute_name][0] = 0.0
        plot(iter_info, min_max_concns, j, last_iter)
        sim.clean_up()
