...
output_path = os.path.join(input_path, 'figure_4_life_history_ABC_10D.pdf')
...
paths = ['T010AS12OBConstEnv10D_Ind_life_history_results.xml', 'T010S12OBConstEnvCleggComp_life_history_results.xml']
...
for i in range(2):
    ...
    while j < len(t)-1:
        ...
        while (j < len(t)-1) and [gen[j]] == [gen[j+1]]:    
            ...
            lasty = [float(( age / (1 - age))  * ( math.sqrt((Yr / muS) * (1 / (1 - age))) - 1))]
            j += 1
...
lastx=[24,240]
lasty=[lasty, lasty]
axisC.plot(lastx,lasty,colorsC[1])
...
cax.text(0.25, 0.695, 'Asymmetric, Dynamic', fontsize=fs)
...
cax.text(0.25, 0.385, 'Symmetric, Dynamic', fontsize=fs)
