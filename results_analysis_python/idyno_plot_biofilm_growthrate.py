...
parser.add_option("-i", "--IterateNum", dest="iter_num", default=7833,
...
if options.colorscheme==1:
    species_color_dict = {'OldieA' : 'cool', 'OldieB' : 'hsv', 'OldieC' : 'cool'}
else:
    species_color_dict = {'OldieA' : 'hsv', 'OldieB' : 'summer', 'OldieC' : 'hsv'}
...
def plot(sim, plot):
    ...
    for i in range(TP):
        ...
        if plot == numplots-1 and i == TP-1:
            ...
            save_path = os.path.join(sim.figures_dir, save_name+'_'+save_num+'_growthrate'+options.file_ext)
